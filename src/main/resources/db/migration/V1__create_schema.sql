...
CREATE TABLE rescue_cases (
    ...
    CONSTRAINT ck_rescue_cases_status CHECK(status IN(
                                                      'ADMITTED',
                                                      'UNDER_EVALUATION',
                                                      'IN_REHABILITATION',
                                                      'READY_FOR_RELEASE',
...
CREATE TABLE animals (
    ...
    CONSTRAINT ck_animals_sex CHECK(sex IN (
                                            'MALE',
                                            'FEMALE',
                                            'UNKNOWN'
                                           )
...
CREATE TABLE medical_records (
    ...
    initial_condition   VARCHAR(50)    NOT NULL,
    injuries            TEXT,
    observations        TEXT,
...
CREATE TABLE specialist_expertise (
    ...
    CONSTRAINT fk_spec_exp_specialist FOREIGN KEY (specialist_id) REFERENCES specialists (id),
    CONSTRAINT fk_spec_exp_expertise FOREIGN KEY (expertise_id) REFERENCES expertise (id)
);
...
CREATE TABLE treatments (
    ...
    CONSTRAINT ck_treatments_type CHECK (type IN (
                                                  'WOUND_CARE',
                                                  'HYDRATION',
                                                  'MEDICATION',
                                                  'SURGERY',
